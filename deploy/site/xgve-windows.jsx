...
(function () {
  ...
  function RetroWindow({ title, x0, y0, w, onClose, children }) {
    ...
    return h("div", { style: { position: "fixed", left: p.x, top: p.y, width: w, zIndex: 90, background: "#070709", border: "1px solid #33363d", boxShadow: "0 12px 40px rgba(0,0,0,.6)", fontFamily: MONO, animation: "xgwin-in .16s ease-out" } },
      h("div", Object.assign({}, dragProps, { style: Object.assign({ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderBottom: "1px solid #33363d", userSelect: "none" }, dragProps.style) }),
        h("button", { onClick: onClose, title: "Close", className: "xgwin-close", style: { width: 11, height: 11, flex: "none", background: "transparent", border: "1px solid #83848c", cursor: "pointer", padding: 0 } }),
    ...
    return h(RetroWindow, { title: "FOXGLOVE_MULTIVERSAL.APP", x0: Math.max(12, window.innerWidth - 560), y0: 80, w: Math.min(480, window.innerWidth - 24), onClose: onClose },
      h("div", { style: { display: "flex", gap: 18, padding: 16 } },
        ...
        h("div", { style: { flex: 1, minWidth: 0, paddingTop: 8 } },
          ...
          h(WISHLIST_URL ? "a" : "div", { href: WISHLIST_URL || undefined, target: WISHLIST_URL ? "_blank" : undefined, style: { display: "inline-block", background: "#c11722", color: "#fff", fontSize: 10, letterSpacing: "0.16em", padding: "6px 10px", marginBottom: 18, textDecoration: "none", cursor: WISHLIST_URL ? "pointer" : "default" } }, WISHLIST_URL ? "\u25cf WISHLIST" : "\u25cf IN DEVELOPMENT"),
          h("div", { style: { fontSize: 12, lineHeight: 2, color: "#6f7480" } },
            h("div", null, "Clock in."),
            h("div", { style: { color: "#9a4a52" } }, "Roll back."),
            h("div", null, "Clock in again."))
        )
      )
    );
  }
  ...
  function ReadmeWindow({ onClose }) {
    return h(RetroWindow, { title: "readme.txt", x0: Math.max(12, Math.min(70, window.innerWidth - 372)), y0: Math.max(60, window.innerHeight - 360), w: Math.min(360, window.innerWidth - 24), onClose: onClose },
      h("div", { style: { padding: "18px 20px 22px", fontSize: 12.5, lineHeight: 1.95, color: "#c6ced8" } },
        h("div", null, "XGVE is a vibecoded games studio."),
        h("div", null, "We build our own worlds \u2014 and ship"),
        h("div", null, "client apps in days, not quarters."),
